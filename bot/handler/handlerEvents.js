...
async function onStart() {
  try {
    ...
    if (GoatBot.config.ownerOnly.enable && senderID !== GoatBot.config.ownerBot[0]) {
      // If ownerOnly mode is enabled and sender is not owner, ignore the command.
      return;
    }
    ...
    if (needRole > role) {
      if (!hideNotiMessage.needRoleToUseCmd) {
        if (needRole == 1)
          await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdmin", commandName));
        else if (needRole == 2)
          await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminBot2", commandName));
      }
      return false;
    }
    ...
    if (timestamps[senderID] && dateNow < timestamps[senderID] + cooldownCommand) {
      const remainingTime = ((timestamps[senderID] + cooldownCommand - dateNow) / 1000).toFixed(1);
      await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "waitingForCommand", remainingTime));
      return false;
    }
    ...
    await command.onStart({
      ...
      commandName,
      getLang: getText2,
      removeCommandNameFromBody: parameters.removeCommandNameFromBody // Use the one from parameters
    });

    log.info("CALL COMMAND", `${commandName} | ${userData.name} | ${senderID} | ${threadID} | ${args.join(" ")}`);
    return true; // Standardized return for success
  } catch (err) {
    log.err("CALL COMMAND", `Error in ${commandName}:`, err);
    await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "errorOccurred", getTime("DD/MM/YYYY HH:mm:ss"), commandName, removeHomeDir(err.stack)));
    return false;
  }
}
